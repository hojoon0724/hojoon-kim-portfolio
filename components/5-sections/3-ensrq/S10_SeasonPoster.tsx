...
const PATH = "/ensrq/s10/brochure-pages";
const POSTER_PAGES = [
...
const POSTER_PAGE_DETAILS: Record<string, posterDetails> = {
  "s10-season-poster_01-cover.png": {
    width: 1800,
    height: 2700,
    widthRatio: 2,
...
const LAST_PAGE_INDEX = POSTER_PAGES.length - 1;
const getSrc = (index: number) => `${PATH}/${POSTER_PAGES[index]}`;
const getPage = (index: number) => POSTER_PAGE_DETAILS[POSTER_PAGES[index]];
...
function PosterImage({ pageIndex, alt }: { pageIndex: number; alt: string }) {
  return (
    <Image
      src={getSrc(pageIndex)}
      alt={alt}
      fill
      sizes={IMAGE_SIZES}
      loading="eager"
...
function FramePage({
  ...
  const { widthRatio, heightRatio } = getPage(pageIndex);
  ...
  return (
    ...
    >
      ...
      >
        <PosterImage pageIndex={pageIndex} alt="" />
      </div>
    </div>
  );
}
...
export function S10_SeasonPoster() {
  const dev = false;
  const posterSrcs = POSTER_PAGES.map((_, index) => getSrc(index));
  ...
  return (
    <div className="s10-poster-container h-full w-full">
      {dev && (
